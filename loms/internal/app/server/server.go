...
import (
	"awesomeProject4/loms/internal/pkg/model"
	orderrepository "awesomeProject4/loms/internal/pkg/repository/order"
	stockrepository "awesomeProject4/loms/internal/pkg/repository/stock"
	"awesomeProject4/loms/internal/pkg/service"
...
type Server struct {
	loms.UnimplementedLOMSServiceServer
	service LomsService
}
...
func (s *Server) OrderInfo(ctx context.Context, req *loms.OrderInfoRequest) (*loms.OrderInfoResponse, error) {
	...
	for _, item := range order.Items {
		res.Items = append(res.Items, &loms.Item{
			Sku:   uint32(item.Sku),
